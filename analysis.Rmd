...
data_means <-
  ...
  summarise(sum_prop_dmg = sum(propertydamage, na.rm=TRUE),
            ...
            mean_prop_dmg = mean(propertydamage, na.rm=TRUE), 
            mean_crop_dmg = mean(cropdamage, na.rm=TRUE),
            sum_injuries = sum(INJURIES, na.rm = TRUE),
            sum_fatalities = sum(INJURIES, na.rm=TRUE),
...
top_prop_dmg <-
  data_means %>%
  select(EVTYPE, mean_prop_dmg) %>%
  arrange(desc(mean_prop_dmg)) %>%
  top_n(5)

g <- ggplot(top_prop_dmg, aes(x=reorder(EVTYPE, -mean_prop_dmg), y=mean_prop_dmg))
g + geom_bar(stat="identity") + xlab("Weather Event Type") + ylab("Mean Property Damage (In US Dollars)") + ggtitle("5 Highest Average Property Damaging Weather Events")



top_crop_dmg <-
  data_means %>%
  select(EVTYPE, mean_crop_dmg) %>%
  arrange(desc(mean_crop_dmg)) %>%
  top_n(5)

g <- ggplot(top_crop_dmg, aes(x=reorder(EVTYPE, -mean_crop_dmg), y=mean_crop_dmg))
g + geom_bar(stat="identity") + xlab("Weather Event Type") + ylab("Mean Crop Damage (In US Dollars)") + ggtitle("5 Highest Average Crop Damaging Weather Events")
```