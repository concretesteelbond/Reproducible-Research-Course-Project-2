...
Cleaning the dataset is quite a lengthy process. There are a lot of different EVTYPE entries for the same type of weather event but use different phrasing/spelling errors. This part of the preprocessing aims to group together as many of these event types as possible. 
```{r cleandata}
clean_data <- 
  raw_data %>%
  ...
  filter(EVTYPE != grepl("summary", EVTYPE)) %>%########################### <<- Fix this
...
# Filter by the 50 most numerous EVTYPEs in dataset
...
top_counts <- ev_df$type[1:50]

data <- filter(clean_data, EVTYPE %in% top_counts)
```

```{r testing}

# Get the average property damage 
data_means <-
  data %>%
  group_by(EVTYPE) %>%
  summarise(mean_prop_dmg = mean(propertydamage, na.rm=TRUE), 
            mean_crop_dmg = mean(cropdamage, na.rm=TRUE),
            total_mean_dmg = mean_prop_dmg + mean_crop_dmg,
